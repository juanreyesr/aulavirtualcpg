...
const getYouTubeThumbnail = (youtubeId, quality = 'hqdefault') => {
  ...
  if (!safeId) return "https://via.placeholder.com/640x360";
...
export default function App() {
  ...
  const incrementViewCount = async (videoId) => {
    const updatedVideos = videos.map(v => v.id === videoId ? { ...v, viewCount: (v.viewCount || 0) + 1 } : v);
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white font-sans selection:bg-blue-600 selection:text-white overflow-x-hidden">
      {/* Navbar — fondo sólido, no transparente */}
      <nav className="fixed top-0 w-full z-50 bg-[#0e0e0e] border-b border-gray-800 px-4 py-3 flex justify-between items-center shadow-lg">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {view !== 'home' && (
            <button onClick={() => setView('home')} className="text-sm hover:text-blue-400 transition-colors mr-1">
              Inicio
            </button>
          )}
          {/* Nuevo botón Avales CAEDUC */}
          <a
            href="https://gestionescaeduc.vercel.app/"
            target="_blank"
            rel="noreferrer"
            className="hidden md:flex items-center gap-1 text-xs text-gray-300 hover:text-white border border-gray-600 px-3 py-1.5 rounded-full hover:bg-gray-800 transition"
          >
            <ExternalLink size={12} /> Avales CAEDUC
          </a>
          <a
            href="https://caeducgt.org/"
            target="_blank"
            rel="noreferrer"
            className="hidden md:flex items-center gap-1 text-xs text-gray-300 hover:text-white border border-gray-600 px-3 py-1.5 rounded-full hover:bg-gray-800 transition"
          >
          ...
          <a
            href="https://colegiodepsicologos.org.gt"
            target="_blank"
            rel="noreferrer"
            className="hidden md:flex items-center gap-1 text-xs text-gray-300 hover:text-white border border-gray-600 px-3 py-1.5 rounded-full hover:bg-gray-800 transition"
          >
...
function HomeView({ videos, recentVideos, categories, upcomingVideos, activities, onVideoSelect }) {
  ...
  return (
    <div className="pb-10">
      {/* Hero Section — más pequeño (50vh en vez de 70vh) */}
      {!activeCategory && heroVideo && (
        <div className="relative h-[50vh] w-full overflow-hidden mt-[57px]">
          ...
          <div className="absolute bottom-0 left-0 p-6 md:p-12 max-w-xl z-10 flex flex-col gap-3">
            <span className="text-yellow-500 font-bold tracking-wider text-xs uppercase bg-black/50 w-fit px-2 py-1 rounded border border-yellow-500/30">
              Destacado
            </span>
            <h1 className="text-2xl md:text-4xl font-bold text-white drop-shadow-xl leading-tight">
              {heroVideo.title}
            </h1>
            <p className="text-gray-200 text-sm md:text-base line-clamp-2 drop-shadow-md">
              {heroVideo.description}
            </p>
            <div className="flex gap-3 mt-2">
              <button
                onClick={() => onVideoSelect(heroVideo)}
                className="bg-white text-black px-6 py-2.5 rounded hover:bg-gray-200 font-bold flex items-center gap-2 transition transform hover:scale-105 text-sm"
              >
                <Play fill="black" size={18} /> Ver Ahora
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Si no hay hero (filtro activo), compensar el navbar */}
        ...
        <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center px-4 py-10">
          <div className="bg-[#141414] border border-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
              ...
              <button type="button" onClick={() => setShowCalendar(false)} className="text-gray-400 hover:text-white" aria-label="Cerrar calendario">
                <X size={18} />
              </button>
            </div>
                      ...
                      <div key={activity.id} className="bg-[#1f1f1f] border border-gray-800 rounded-xl p-4 md:p-5">
                        ...
                        <div className="flex flex-wrap gap-4 mt-4">
                          {activity.meetingLink && (
                            <a href={activity.meetingLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-blue-300 hover:text-blue-200">
                              <ExternalLink size={14} /> Enlace de actividad
                            </a>
                          )}
                          {activity.registrationLink && (
                            <a href={activity.registrationLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-blue-300 hover:text-blue-200">
      ...
      {/* Recién Añadidos */}
      {!activeCategory && (
        <div className="pl-8 md:pl-16 mt-8 md:mt-10 relative z-20">
          ...
          <div className="flex gap-4 overflow-x-auto pb-8 pr-8 scrollbar-hide snap-x">
            {recentVideos.map(video => (
              <VideoCard key={video.id} video={video} onClick={() => onVideoSelect(video)} isPublished={isVideoPublished(video)} />
            ))}
          </div>
        </div>
      )}
...
// VideoCard con título siempre visible abajo
function VideoCard({ video, onClick, isSmall, isPublished }) {
  ...
  return (
    <div
      ...
      className={`relative flex-shrink-0 bg-gray-900 rounded-md overflow-hidden transition-all duration-300 transform hover:scale-105 hover:z-50 hover:shadow-2xl hover:shadow-blue-900/40 group flex flex-col ${isSmall ? 'w-56' : 'w-72'} ${isPublished ? 'cursor-pointer' : 'cursor-not-allowed opacity-80'}`}
    >
      {/* Thumbnail */}
      <div className={`relative flex-shrink-0 ${isSmall ? 'h-32' : 'h-40'} w-full`}>
        ...
        {/* Contador de visitas */}
        {isPublished && (
          <div className="absolute top-2 left-2 bg-black/70 px-2 py-0.5 text-xs rounded text-white flex items-center gap-1">
        ...
        {!isPublished && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70 text-center px-4">
            <span className="text-yellow-400 font-bold text-xs uppercase tracking-widest">Próximamente</span>
            {scheduledLabel && (
              <span className="text-xs text-gray-200 mt-1">Disponible el {scheduledLabel}</span>
            )}
          </div>
        )}
      </div>

      {/* Título y meta — siempre visible */}
      <div className="bg-[#1a1a1a] px-3 py-2.5 flex flex-col gap-0.5">
      ...
      <div className="max-w-2xl mx-auto bg-gray-900 p-8 rounded-lg border border-gray-800 shadow-2xl">
        ...
        <div className="space-y-4 mb-8">
          <div>
            ...
            <input type="text" value={userProfile.name} onChange={e => setUserProfile({ ...userProfile, name: e.target.value })} className="w-full bg-black border border-gray-700 rounded p-3 text-white focus:border-blue-500 outline-none" placeholder="Ej. Lic. Juan Pérez" />
          </div>
          <div>
            <label className="block text-gray-400 text-sm mb-1">Número de Colegiado</label>
            <input type="text" value={userProfile.collegiateNumber} onChange={e => setUserProfile({ ...userProfile, collegiateNumber: e.target.value })} className="w-full bg-black border border-gray-700 rounded p-3 text-white focus:border-blue-500 outline-none" placeholder="Ej. 12345" />
          </div>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto bg-gray-900 p-6 md:p-8 rounded-lg border border-gray-800">
      ...
      <div className="mt-8 flex justify-end gap-4 border-t border-gray-800 pt-4">
        <button onClick={onCancel} className="text-gray-400 hover:text-white">Cancelar</button>
        <button onClick={handleSubmit} disabled={Object.keys(answers).length < 10} className="bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-8 py-2 rounded font-bold transition">
    ...
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="overflow-auto w-full flex justify-center p-4">
        <div ref={certRef} className="relative shadow-2xl" style={{ width: '1056px', height: '816px', fontFamily: "'Georgia', 'Times New Roman', serif" }}>
          <img src="/certificado-plantilla.png" alt="Plantilla Certificado" className="absolute inset-0 w-full h-full object-cover" crossOrigin="anonymous" onLoad={() => setImageLoaded(true)} onError={(e) => { console.error('Error cargando plantilla'); e.target.style.display = 'none'; setImageLoaded(true); }} />
          <div className="absolute inset-0">
            <div className="absolute text-center" style={{ top: '222px', left: '50%', transform: 'translateX(-50%)', width: '650px' }}>
              <p className="font-bold text-[#1a1a2e]" style={{ fontSize: '38px', fontFamily: "'Georgia', serif", letterSpacing: '0.5px' }}>{userProfile.name}</p>
            </div>
            <div className="absolute" style={{ top: '302px', left: '595px' }}>
              <p className="text-[#1a1a2e] font-semibold" style={{ fontSize: '16px', fontFamily: "'Georgia', serif" }}>{userProfile.collegiateNumber}</p>
            </div>
            <div className="absolute text-center" style={{ top: '388px', left: '50%', transform: 'translateX(-50%)', width: '750px' }}>
              <p className="font-bold text-[#1a1a2e] uppercase" style={{ fontSize: '24px', fontFamily: "'Georgia', serif", lineHeight: '1.3' }}>{video.title}</p>
            </div>
            <div className="absolute" style={{ top: '448px', left: '445px' }}>
              <p className="text-[#1a1a2e] font-bold" style={{ fontSize: '18px', fontFamily: "'Georgia', serif" }}>{video.duration}</p>
            </div>
            <div className="absolute" style={{ top: '565px', left: '50%', transform: 'translateX(-50%)' }}>
              <p className="text-[#333]" style={{ fontSize: '14px', fontFamily: "'Georgia', serif" }}>{currentDate.toLocaleDateString('es-GT', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
            </div>
            <div className="absolute" style={{ bottom: '95px', right: '50px' }}>
              <p className="text-[#333] font-medium" style={{ fontSize: '11px', fontFamily: "'Courier New', monospace", letterSpacing: '0.5px' }}>{certificateCode}</p>
            </div>
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-between text-black bg-white border-8 border-double border-[#003366] p-8" style={{ display: imageLoaded ? 'none' : 'flex', fontFamily: "'Georgia', 'Times New Roman', serif" }}>
            ...
            <div className="text-center flex-1 flex flex-col justify-center gap-4">
              <h2 className="text-4xl font-bold text-[#003366] border-b-2 border-[#d4af37] pb-2 px-8">{userProfile.name}</h2>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black/90 px-4">
      <div className="w-full max-w-md bg-[#141414] p-8 rounded-lg shadow-2xl border border-gray-800 relative">
        ...
        {authError && (
          <div className="mb-6 rounded border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-200">{authError}</div>
        )}
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-gray-400 text-sm mb-2">Correo Electrónico</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-3 rounded bg-[#333] text-white border-none focus:ring-2 focus:ring-blue-600 outline-none" required />
          </div>
          <div>
            <label className="block text-gray-400 text-sm mb-2">Contraseña</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-3 rounded bg-[#333] text-white border-none focus:ring-2 focus:ring-blue-600 outline-none" required />
          </div>
  ...
  return (
    <div className="bg-gray-800 p-4 rounded mb-4 border border-gray-700">
      <div className="mb-2">
        ...
        <input type="text" value={question.question} onChange={(e) => onQuestionChange(idx, (current) => ({ ...current, question: e.target.value }))} className="w-full bg-gray-900 border border-gray-600 rounded p-2 text-sm text-white" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        {question.options.map((opt, optIdx) => (
          <div key={optIdx} className="flex flex-col">
            <input type="text" value={opt} onChange={(e) => { onQuestionChange(idx, (current) => { const newOpts = [...current.options]; newOpts[optIdx] = e.target.value; return { ...current, options: newOpts }; }); }} className={`w-full bg-gray-900 border rounded p-2 text-xs text-white ${question.correctAnswer === optIdx ? 'border-green-500 ring-1 ring-green-500' : 'border-gray-600'}`} placeholder={`Opción ${optIdx + 1}`} />
            <label className="flex items-center gap-1 mt-1 text-xs text-gray-400 cursor-pointer">
              <input type="radio" name={`correct-${idx}`} checked={question.correctAnswer === optIdx} onChange={() => onQuestionChange(idx, (current) => ({ ...current, correctAnswer: optIdx }))} /> Correcta
            </label>
          </div>
        ))}
      </div>
...
function AdminDashboard({ videos, activities, onVideosChange, onActivitiesChange, onGenerateCertificate }) {
  ...
  const handleEdit = (video) => {
    ...
    setFormData({ ...video, scheduledAt: video.scheduledAt || '', thumbnail: video.thumbnail || '' });
    setQuestions((video.questions || []).map((question) => ({ ...question, options: [...(question.options || [])] })));
  };

  const handleCreate = () => {
    setSaveError('');
    const empty = { id: Date.now(), title: '', category: '', youtubeId: '', duration: '', description: '', thumbnail: '', scheduledAt: '', quizEnabled: false, viewCount: 0 };
    setEditingVideo(empty);
    setFormData(empty);
    setQuestions(Array(10).fill(null).map((_, i) => ({ question: `Pregunta ${i + 1}`, options: ["Opción 1", "Opción 2", "Opción 3"], correctAnswer: 0 })));
  };

  const updateQuestion = useCallback((idx, updater) => {
    setQuestions((prev) => prev.map((question, index) => index !== idx ? question : updater(question)));
  }, []);
  ...
  const handleSave = async () => {
    ...
    setSaveError('');
    try {
      if (videos.some(v => v.id === newVideo.id)) await onVideosChange(videos.map(v => v.id === newVideo.id ? newVideo : v));
      else await onVideosChange([...videos, newVideo]);
  ...
  const handleDelete = async (id) => {
    if (confirm("¿Estás seguro de eliminar este video?")) {
      setSaveError('');
      try { await onVideosChange(videos.filter(v => v.id !== id)); }
      catch (error) { setSaveError(`No se pudo eliminar el video: ${error.message}`); }
    }
  };
  ...
  const handleActivityEdit = (activity) => {
    ...
    setActivityForm({ title: activity.title || '', organizer: activity.organizer || '', date: activity.date || '', time: activity.time || '', location: activity.location || '', registrationLink: activity.registrationLink || '', meetingLink: activity.meetingLink || '', isFull: Boolean(activity.isFull) });
  };

  const handleActivitySave = async () => {
    if (!activityForm.title || !activityForm.date) { setActivityError('El título y la fecha de la actividad son obligatorios.'); return; }
    setActivityError('');
    const nextActivity = { ...editingActivity, ...activityForm };
    try {
      const exists = activities.some(activity => activity.id === nextActivity.id);
      const nextActivities = exists ? activities.map(activity => activity.id === nextActivity.id ? nextActivity : activity) : [...activities, nextActivity];
      await onActivitiesChange(nextActivities);
      setEditingActivity(null);
      setActivityForm({ title: '', organizer: '', date: '', time: '', location: '', registrationLink: '', meetingLink: '', isFull: false });
    } catch (error) { setActivityError(`No se pudo guardar la actividad: ${error.message}`); }
  };

  const handleActivityDelete = async (id) => {
    if (!confirm("¿Eliminar esta actividad?")) return;
    setActivityError('');
    try { await onActivitiesChange(activities.filter(activity => activity.id !== id)); }
    catch (error) { setActivityError(`No se pudo eliminar la actividad: ${error.message}`); }
  };
  ...
  const handleReportGenerate = () => {
    ...
    if (Number.isNaN(startDate.valueOf()) || Number.isNaN(endDate.valueOf())) { setReportError('El rango de fechas no es válido.'); return; }
    if (endDate < startDate) { setReportError('La fecha final debe ser posterior a la fecha inicial.'); return; }
    const filteredActivities = activities.filter(a => a?.date).map(a => ({ ...a, parsedDate: new Date(`${a.date}T00:00:00`) })).filter(a => !Number.isNaN(a.parsedDate.valueOf())).filter(a => a.parsedDate >= startDate && a.parsedDate <= endDate);
    if (filteredActivities.length === 0) { setReportError('No hay actividades registradas en este rango.'); return; }
    const escapeCsv = (value) => { if (value === null || value === undefined) return ''; const s = String(value); if (s.includes('"') || s.includes(',') || s.includes('\n')) return `"${s.replace(/"/g, '""')}"`; return s; };
    const headers = ['Título', 'Organizador', 'Fecha', 'Hora', 'Lugar', 'Cupo lleno', 'Enlace actividad', 'Enlace inscripción'];
    const rows = filteredActivities.map(a => ([a.title, a.organizer || 'Por definir', a.date, a.time || 'Por confirmar', a.location || 'Por confirmar', a.isFull ? 'Sí' : 'No', a.meetingLink || '', a.registrationLink || '']));
  ...
  const handleManualCertOpen = (video) => { setManualCertVideo(video); setManualProfile({ name: '', collegiateNumber: '' }); };
  const handleManualCertGenerate = () => {
    if (!manualProfile.name || !manualProfile.collegiateNumber) { alert("Por favor ingrese el nombre del profesional y el número de colegiado."); return; }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-gray-900 p-6 rounded border border-gray-800">
            <div className="flex items-center gap-3 mb-4">
              <input type="checkbox" id="quizToggle" checked={formData.quizEnabled} onChange={e => setFormData({ ...formData, quizEnabled: e.target.checked })} className="w-5 h-5 text-blue-600 rounded" />
    ...
    <div className="min-h-screen bg-[#141414] pt-24 px-4 md:px-16 text-white">
      ...
      <div className="bg-[#1b1b1b] border border-gray-800 rounded-2xl p-6 mb-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          ...
          <button type="button" onClick={() => { setReportError(''); setShowReportModal(true); }} className="inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 px-4 py-2 rounded font-semibold text-sm">Informe de actividades</button>
        </div>
        {editingActivity !== null && (
          <div className="bg-[#141414] border border-gray-800 rounded-xl p-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label className="block text-sm text-gray-400 mb-1">Enlace de la actividad (Zoom/Meet)</label><input type="url" value={activityForm.meetingLink} onChange={(e) => setActivityForm({ ...activityForm, meetingLink: e.target.value })} className="w-full bg-black border border-gray-700 rounded p-2 text-white" /></div>
              <div><label className="block text-sm text-gray-400 mb-1">Enlace de inscripción (opcional)</label><input type="url" value={activityForm.registrationLink} onChange={(e) => setActivityForm({ ...activityForm, registrationLink: e.target.value })} className="w-full bg-black border border-gray-700 rounded p-2 text-white" /></div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => { setEditingActivity(null); setActivityForm({ title: '', organizer: '', date: '', time: '', location: '', registrationLink: '', meetingLink: '', isFull: false }); }} className="px-4 py-2 bg-gray-700 rounded hover:bg-gray-600">Cancelar</button>
          ...
          {activities.map((activity) => (
            <div key={activity.id} className="bg-[#141414] border border-gray-800 rounded-xl p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                {activity.isFull && <span className="text-xs uppercase tracking-wide bg-red-500/20 text-red-200 border border-red-500/40 px-2 py-1 rounded-full">Cupo lleno</span>}
              </div>
              ...
              {activity.meetingLink && <a href={activity.meetingLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-3 text-sm text-blue-300 hover:text-blue-200"><ExternalLink size={14} /> Enlace de actividad</a>}
              {activity.registrationLink && <a href={activity.registrationLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-3 text-sm text-blue-300 hover:text-blue-200"><ExternalLink size={14} /> Inscripción</a>}
              <div className="flex gap-2 mt-4">
                <button onClick={() => handleActivityEdit(activity)} className="flex-1 bg-blue-900/40 hover:bg-blue-900/60 text-blue-200 py-2 rounded text-sm transition">Editar</button>
                <button onClick={() => handleActivityDelete(activity.id)} className="px-3 bg-red-900/40 hover:bg-red-900/60 text-red-300 rounded transition"><Trash2 size={16} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-black/70 z-[70] flex items-center justify-center px-4 py-10">
          <div className="bg-[#141414] border border-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
              <div><h3 className="text-lg font-bold text-white">Informe de actividades</h3><p className="text-sm text-gray-400">Selecciona el rango de fechas para el informe.</p></div>
              <button type="button" onClick={() => setShowReportModal(false)} className="text-gray-400 hover:text-white"><X size={18} /></button>
            </div>
            <div className="px-6 py-6 space-y-4">
              {reportError && <div className="rounded border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-200">{reportError}</div>}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-sm text-gray-400 mb-1">Desde</label><input type="date" value={reportRange.start} onChange={(e) => setReportRange({ ...reportRange, start: e.target.value })} className="w-full bg-black border border-gray-700 rounded p-2 text-white" /></div>
                <div><label className="block text-sm text-gray-400 mb-1">Hasta</label><input type="date" value={reportRange.end} onChange={(e) => setReportRange({ ...reportRange, end: e.target.value })} className="w-full bg-black border border-gray-700 rounded p-2 text-white" /></div>
              </div>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-800">
              <button type="button" onClick={() => setShowReportModal(false)} className="px-4 py-2 bg-gray-700 rounded hover:bg-gray-600">Cancelar</button>
              <button type="button" onClick={handleReportGenerate} className="px-4 py-2 bg-emerald-600 rounded hover:bg-emerald-700 font-bold">Descargar informe</button>
            </div>
          </div>
        </div>
      )}
        ...
        {videos.map(video => (
          <div key={video.id} className="bg-gray-900 rounded-lg overflow-hidden border border-gray-800 flex flex-col">
            <div className="h-40 relative">
              <img src={getVideoThumbnail(video)} className="w-full h-full object-cover" alt="" onError={(e) => { const target = e.currentTarget; const stage = target.dataset.fallbackStage || 'hqdefault'; if (stage === 'hqdefault') { target.dataset.fallbackStage = 'mqdefault'; target.src = getYouTubeThumbnail(video.youtubeId, 'mqdefault'); return; } if (stage === 'mqdefault') { target.dataset.fallbackStage = 'placeholder'; target.src = getYouTubeThumbnail(''); } }} />
            ...
            <div className="p-4 flex-1">
              ...
              <div className="flex items-center gap-2 text-xs mb-4 flex-wrap">
                ...
                {!isVideoPublished(video) && <span className="text-yellow-400 border border-yellow-400/30 px-2 py-0.5 rounded">Programado {formatScheduleDate(video.scheduledAt)}</span>}
              </div>
            </div>
            <div className="p-4 border-t border-gray-800 flex gap-2">
              ...
              {video.quizEnabled && <button onClick={() => handleManualCertOpen(video)} className="flex-1 bg-yellow-700/40 hover:bg-yellow-700/60 text-yellow-200 py-2 rounded text-sm transition">Generar Certificado</button>}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-700 rounded-lg p-6 w-full max-w-lg">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button onClick={handleManualCertGenerate} className="px-4 py-2 bg-yellow-600 rounded hover:bg-yellow-700 font-bold">Generar</button>
            </div>
          </div>
        </div>
      )}
